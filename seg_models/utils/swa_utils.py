...
def swa(load_model, model, model_folder, dataset, batch_size, device, epoch_list, num_fold):
    directory = Path(model_folder)
    model_files = [f for f in directory.iterdir() 
               if any(str(f).endswith(f"model_epoch_{epoch}_{num_fold}.pth.tar") for epoch in epoch_list)]
