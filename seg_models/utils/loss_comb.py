...
class HausdorffLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        """
        Approximates the Hausdorff Distance between prediction and target masks.
        outputs: raw logits or probabilities (before thresholding)
        targets: binary ground truth masks
        """

        outputs = torch.sigmoid(outputs)  # ensure probabilities
        outputs = outputs.contiguous().view(outputs.shape[0], -1)
        ...
        # Approximate Hausdorff by (1 - intersection / union)
        # (This is a rough differentiable proxy, not exact Hausdorff.)
        union = outputs_sum + targets_sum - intersection
        hausdorff_like = 1 - (intersection + 1e-6) / (union + 1e-6)

        return hausdorff_like.mean()
